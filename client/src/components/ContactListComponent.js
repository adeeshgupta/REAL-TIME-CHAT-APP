...
import utility from "../managers/validateEmail";
...
const ContactComponent = (props) => {
  ...
  const otherUser = userData.channelUsers?.find((userObj) => userObj.email !== userInfo.email) || userData;
  ...
  return (
    <ContactItem onClick={() => setChat({ channelData: userData, otherUser })}>
      ...
      <MessageTime>{lastMessage && new Date(lastMessage?.addedOn).toLocaleString()}</MessageTime>
    </ContactItem>
  );
};
...
function ContactListComponent(props) {
  ...
  const onSearchTextChanged = async (searchText) => {
    ...
    if (!utility.validateEmail(searchText))
      return;
  ...
  return (
    <Container>
      <ProfileInfoDiv>
        <ProfileImage src={userInfo.picture || "/images/adeesh.jpeg"} />
        <CurrName>{userInfo.name}</CurrName>
      </ProfileInfoDiv>
      <SearchBox>
        <SearchContainer>
          <SearchIcon src={"/images/search-icon.svg"} />
          <SearchInput placeholder="Search or start new chat" value={searchString}
      ...
      {contactList.map((userData) => (
        <ContactComponent userData={userData} setChat={props.setChat} userInfo={userInfo} />
      ))}
    </Container>
  );
}
